...
public class IntervalComplication: ComplicationPresentable {
    ...
    public func startMonitoring() {
        NotificationCenter.default.publisher(for: name).sink { notification in
            if let interval = notification.userInfo?[IntervalTracker.intervalKey] as? TimeInterval {
...
public class IntervalTracker {
    ...
    public func track(_ type: SignpostType) {
        ...
        case .end:
            ...
            NotificationCenter.default.post(Notification(name: notificationName, object: nil, userInfo: [IntervalTracker.intervalKey : interval]))
        }
    }
}
